// 为用户增加自定义数量Logo生成额度的脚本，从项目根目录的.env.local读取Redis凭证
// 使用方法: node scripts/add-credits.js <用户ID> <额度数量>
...
if (process.argv.length < 4) {
  console.error('用法: node scripts/add-credits.js <用户ID> <额度数量>');
  console.error('例如: node scripts/add-credits.js user_123456 20  # 添加20次额度');
  process.exit(1);
}
...
if (isNaN(creditsToAdd) || creditsToAdd <= 0) {
  console.error('错误: 额度数量必须是大于0的数字');
  process.exit(1);
}
...
async function addCredits() {
  try {
    console.log(`正在为用户 ${userId} 添加 ${creditsToAdd} 次额度...`);
    ...
    if (!currentLimitData) {
      console.log('未找到该用户的限流记录，将创建新记录并添加额度');
      
      // 创建新的限流记录，设置为允许用户有指定次数的使用机会
      // 格式：[时间窗口开始时间戳, 已使用次数]
      const newLimitData = [Date.now(), -creditsToAdd]; // 负值表示用户还有额度可用
      await redis.set(rateLimitKey, JSON.stringify(newLimitData));
      
      console.log(`成功为用户 ${userId} 添加了 ${creditsToAdd} 次生成额度`);
      return;
    }
    ...
    const newUsage = currentUsage - creditsToAdd;
