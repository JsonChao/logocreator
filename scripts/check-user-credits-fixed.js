// 查询用户当前Logo生成额度状态的脚本，从项目根目录的.env.local读取Redis凭证
// 使用方法: node scripts/check-user-credits-fixed.js <用户ID>
...
if (process.argv.length < 3) {
  console.error('用法: node scripts/check-user-credits-fixed.js <用户ID>');
  process.exit(1);
}
...
async function checkUserCredits() {
  try {
    ...
    if (!currentLimitData) {
      console.log(`未找到用户 ${userId} 的限流记录，默认有3次免费生成额度`);
      return;
    }
